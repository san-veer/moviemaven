...
function MovieDesc() {
  ...
  const postSimilarMovies = async (movie) => {
    try {
      ...
      console.log("res1 and 2 in similar movies:", res1, res2);
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const response = await axios.get(`https://af2c-203-192-244-153.ngrok-free.app/api/v1/movie/${id}`);
      ...
      {similarLoader && similarMovie.watch_history?.length > 0 ?
        ...
        <div style={{ display: "flex", flexDirection: "row", flexWrap: "wrap", paddingLeft: 100 }} className="movie-list" >
          {similarMovie?.watch_history?.map((movie) => (
            <NavLink className="no-underline" style={{ textDecoration: 'none', width: 230, marginBottom:23 }} to={`/movie/${movie._id}`} key={movie._id}>
